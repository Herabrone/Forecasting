...
# Process data: get the timer series, convert to floats, get a single column of result values, return it as an array
def process(days):

    # Convert to continuous
    days_continuous = days.astype(float)

    # (Insert a bunch of super cool machine learning stuff here)

    # Result column (as a placeholder it just gets the mean of each row)
    result_col = days_continuous.mean(axis = 1)

    # Convert result column back to integers, make sure it's nonnegative
    result_col_int = result_col.clip(lower = 0).round().astype(int)

    return result_col_int.to_numpy() # Return as an array
...
if __name__ == '__main__':
    ...
    result = process(days) # Process the data
    print(result)
    print(result.shape)